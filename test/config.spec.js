...
test('CONFIG.exists(pkg) - throw if config already exists', (t) => {
  const pkg = JSON.parse(fixtures.fpmInitData);
  try {
    CONFIG.exists(pkg);
  } catch(e) {
    t.pass('should throw an exception');
    t.equal(e.message, `ERR_CONFIG: Init failed, 'fpmDependencies' already exists in package.json`, 'should have the correct message');
  }
...
test('CONFIG.isInstalled(pkg) - true if the package has not been installed', (t) => {
  const pkg = JSON.parse(fixtures.fpmPackageExistsData);
  const exists = CONFIG.isInstalled(pkg, "wc-markdown");
